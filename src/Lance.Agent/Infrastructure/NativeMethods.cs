...
internal static class NativeMethods
{
    ...
    // Windows: read the full command line of an external process via PEB inspection.

    internal static string? ReadProcessCommandLine(int pid)
    {
        ...
        try
        {
            ...
            // x64 → offset 0x70; layout: Length(2) + MaxLength(2) + pad(4) + Buffer(8) = 16 bytes
            // x86 → offset 0x40; layout: Length(2) + MaxLength(2) + Buffer(4) = 8 bytes
            ...
            // Buffer pointer sits at byte 8 (x64) or 4 (x86) within the UNICODE_STRING
