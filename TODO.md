...
- Performance (stutter and lag) issues - performance tuning required.
  - [ ] End-to-end test of the whole stream-tuning stack
- [FEAT] Lack of UI to jump back to host PC
- [FEAT] Mouse pointer on mixed screen (some with moonlight others with host pc) - pointer goes behind moonlight
...
- [BUG] Multiple CTRL+C to stop the client insead of only one
- [FEAT] resuming session, windows position: https://github.com/kangyu-california/PersistentWindows
- [FEAT] Make a Lance Client GUI
- [FEAT???] Slot config sync

### Feature: Slot config sync
Clone configs drift from the template and there is no way to push a setting to every slot.
